...
class Helper {
    ...
    public function get_hostinger_plugin_url(): string {
        ...
        $reseller_locale = get_option( 'hostinger_reseller', '' );
        $locale          = self::HOSTINGER_LOCALES[ $website_locale ] ?? 'hostinger.com';
        $base_domain     = $reseller_locale ?? $locale;

        $plugin_url  = rtrim( $base_domain, '/' ) . '/';
        $plugin_url .= str_replace( ABSPATH, '', HOSTINGER_EASY_ONBOARDING_ABSPATH );

        return $plugin_url;
    }

    public function is_store_setup_completed(): bool {
    ...
    public function get_store_industry(): array {
        $onboarding_profile = get_option( 'woocommerce_onboarding_profile', array() );
        return $onboarding_profile['industry'] ?? array();
    }
